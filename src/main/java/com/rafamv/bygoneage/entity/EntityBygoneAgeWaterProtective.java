...
public class EntityBygoneAgeWaterProtective extends EntityBygoneAgeWaterCreature {
	...
	public EntityBygoneAgeWaterProtective(World world, byte id, int alliesToAttack, float maxChildAge) {
		super(world, id, maxChildAge);
		this.numberOfAllies = alliesToAttack;
		this.tasks.addTask(1, new BygoneAgeWaterEntityAIPanicIfNotAnger(this, 1.2D));
	...
	private void cancelAngryBehavior(EntityBygoneAgeWaterProtective howWasAttacking) {
		howWasAttacking.setAngerLevel((short) 0);
		howWasAttacking.setAttackTarget((EntityLivingBase) null);
	}

	/** Sets the target of the creature. */
	private void becomeAngryAt(EntityBygoneAgeWaterProtective creature, Entity target) {
	...
	private void becomePanic(EntityBygoneAgeWaterProtective fleeingCreature) {
	...
	public boolean attackEntityFrom(DamageSource damageSource, float damage) {
		if (this.isEntityInvulnerable()) {
			...
			ArrayList<EntityBygoneAgeWaterProtective> listChildren = new ArrayList<EntityBygoneAgeWaterProtective>();
			ArrayList<EntityBygoneAgeWaterProtective> listAdult = new ArrayList<EntityBygoneAgeWaterProtective>();
			...
			for (int i = 0; i < list.size(); ++i) {
				...
				if (entityNeighbor.getClass() == this.getClass()) {
					EntityBygoneAgeWaterProtective validEntityNeighbor = (EntityBygoneAgeWaterProtective) entityNeighbor;
