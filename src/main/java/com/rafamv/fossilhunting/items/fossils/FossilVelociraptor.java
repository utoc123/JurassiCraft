...
public class FossilVelociraptor extends AnyFossilItemModel {

	public FossilVelociraptor() {
	...
	public void registerIcons(IIconRegister iconRegister) {
		this.itemIcon = iconRegister.registerIcon(FossilHunting.MODID + ":" + FossilInformation.VELOCIRAPTOR.getIconName());
	}
	...
	public boolean onItemUse(ItemStack itemStack, EntityPlayer player, World world, int x, int y, int z, int par7, float par8, float par9, float par10) {
		if (!world.isRemote) {
			...
			if (!player.canPlayerEdit(x, y, z, par7, itemStack)) {
				...
				int posX = FossilInformation.VELOCIRAPTOR.getXBound();
				int posZ = FossilInformation.VELOCIRAPTOR.getZBound();
				int direction = MathHelper.floor_double(player.rotationYaw * 4.0F / 360.0F + 0.5D);
				if (direction == 0) {
					posX = FossilInformation.VELOCIRAPTOR.getXBound();
					posZ = FossilInformation.VELOCIRAPTOR.getZBound();
					...
					if (checkPlacementAtSouthBetween(world, itemStack, x, y, z, posX, posZ)) {
						...
						world.setBlock(x, y, z, FHBlocks.blockFHFossilModelVelociraptor);
					...
					posX = FossilInformation.VELOCIRAPTOR.getZBound();
					posZ = FossilInformation.VELOCIRAPTOR.getXBound();
	...
	public void setInformation(World world, EntityPlayer player, ItemStack itemStack, int x, int y, int z, TileEntityFossil fossil) {
		if (!itemStack.hasTagCompound()) {
			fossil.setFossilDataFirstTime(world, x, y, z, player, FossilInformation.VELOCIRAPTOR.getFossilID(), 0.75D, 0.9D);
		} else {
