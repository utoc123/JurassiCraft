...
public class FossilStegosaurus extends AnyFossilItemModel {

	public FossilStegosaurus() {
	...
	public void registerIcons(IIconRegister iconRegister) {
		this.itemIcon = iconRegister.registerIcon(FossilHunting.MODID + ":" + FossilInformation.STEGOSAURUS.getIconName());
	}
			...
			if (!player.canPlayerEdit(x, y, z, par7, itemStack)) {
				...
				if (direction == 0) {
					...
					if (checkPlacementAtSouthBetween(world, itemStack, x, y, z, posX, posZ)) {
						...
						world.setBlock(x, y, z, FHBlocks.blockFHFossilModelStegosaurus);
					...
					posX = FossilInformation.STEGOSAURUS.getZBound();
					posZ = FossilInformation.STEGOSAURUS.getXBound();
	...
	public void setInformation(World world, EntityPlayer player, ItemStack itemStack, int x, int y, int z, TileEntityFossil fossil) {
		if (!itemStack.hasTagCompound()) {
			fossil.setFossilDataFirstTime(world, x, y, z, player, FossilInformation.STEGOSAURUS.getFossilID(), 0.75D, 0.9D);
		} else {
