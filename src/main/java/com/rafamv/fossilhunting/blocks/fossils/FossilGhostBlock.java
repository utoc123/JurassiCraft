...
public class FossilGhostBlock extends Block implements ITileEntityProvider {

	public FossilGhostBlock() {
		super(Material.rock);
	...
	public void setBlockBoundsBasedOnState(IBlockAccess world, int x, int y, int z) {
		if (world.getBlock(x, y + 1, z).getUnlocalizedName().equals(FHBlocks.blockFHFossilGhost.getUnlocalizedName())) {
	...
	public boolean onBlockActivated(World world, int x, int y, int z, EntityPlayer player, int side, float hitX, float hitY, float hitZ) {
		if (!world.isRemote) {
			...
			if (tileentity instanceof TileEntityGhostBlock) {
				...
				return activeBlockAt(world, ghostBlock.getMainPositionX(), ghostBlock.getMainPositionY(), ghostBlock.getMainPositionZ(), player);
			}
		}
		return false;
	}
	...
	@SideOnly(Side.CLIENT)
	private boolean activeBlockAt(World world, int x, int y, int z, EntityPlayer player) {
		if (!world.isRemote) {
	...
	private boolean changeFossilAngle(World world, int x, int y, int z, EntityPlayer player, Item tool) {
	...
	public void breakBlock(World world, int x, int y, int z, Block block, int meta) {
		...
		if (tileEntity instanceof TileEntityGhostBlock) {
			TileEntityGhostBlock ghostBlock = (TileEntityGhostBlock) tileEntity;
			TileEntity tileEntityFossil = world.getTileEntity(ghostBlock.getMainPositionX(), ghostBlock.getMainPositionY(), ghostBlock.getMainPositionZ());
			if (tileEntityFossil instanceof TileEntityFossil) {
				TileEntityFossil fossil = (TileEntityFossil) tileEntityFossil;
				...
				if (direction == ForgeDirection.SOUTH) {
					...
					removeGhostBlocksAtSouthBetween(world, fossil, ghostBlock.getMainPositionX(), ghostBlock.getMainPositionY(), ghostBlock.getMainPositionZ(), posX, posZ);
					...
					removeGhostBlocksAtWestBetween(world, fossil, ghostBlock.getMainPositionX(), ghostBlock.getMainPositionY(), ghostBlock.getMainPositionZ(), posX, posZ);
				} else if (direction == ForgeDirection.NORTH) {
					posX = FossilInformation.values()[fossil.getFossilID()].getXBound();
					posZ = FossilInformation.values()[fossil.getFossilID()].getZBound();
					System.out.println("Direction 2 (North): " + direction);
					removeGhostBlocksAtNorthBetween(world, fossil, ghostBlock.getMainPositionX(), ghostBlock.getMainPositionY(), ghostBlock.getMainPositionZ(), posX, posZ);
				} else if (direction == ForgeDirection.EAST) {
					posX = FossilInformation.values()[fossil.getFossilID()].getZBound();
					posZ = FossilInformation.values()[fossil.getFossilID()].getXBound();
					System.out.println("Direction 3 (East): " + direction);
					removeGhostBlocksAtEastBetween(world, fossil, ghostBlock.getMainPositionX(), ghostBlock.getMainPositionY(), ghostBlock.getMainPositionZ(), posX, posZ);
				}
			}
		}
	}
