...
public class ContainerCultivate extends Container
{
    ...
    public ItemStack transferStackInSlot(EntityPlayer player, int slot)
    {
        ...
        if (slotObject != null && slotObject.getHasStack())
        {
            ...
            if (slot < 2) if (!this.mergeItemStack(stackInSlot, 2, 38, false)) return null;
            //places it into the tileEntity is possible since its in the player inventory
            else if (canCultivate(stackInSlot)) if (!this.mergeItemStack(stackInSlot, 0, 1, false)) return null;
