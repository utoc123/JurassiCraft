...
public class GuiCultivateProcess extends GuiScreen {
	...
	public void drawScreen(int x, int y, float f) {
		...
		if (this.cultivator.getEmbryoID() >= 0) {
			this.fontRendererObj.drawString(StatCollector.translateToLocal("Cultivating: " + JurassiCraftCreatureInformation.values()[this.cultivator.getEmbryoID()].getCreatureName()), this.guiLeft + (this.xSize - this.fontRendererObj.getStringWidth("Cultivating: " + JurassiCraftCreatureInformation.values()[this.cultivator.getEmbryoID()].getCreatureName())) / 2, this.guiTop + 10, 4210752);
